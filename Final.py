...
def encrypt(pword):
    ...
    while i < len(pword):
        ...
        if digitis(char) and char != '0':
            ...
            for x in range(iterations):
                shift = random.randint(0, 94)  # Random shift between 0 and 94
                encrypted_char = alph[shift]
                if x == 0:
                    encrypted_pword += encrypted_char
                else:
            ...
            alph = "abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ~!@#$%^&*()_+=[';/.>,m';]"
            for x in range(iterations):
                shift = random.randint(0, 86)  # Random shift between 0 and 86
                encrypted_char = alph[shift]
                encrypted_pword += encrypted_char
            char_group.append(char)
            char_indexes.append(i)
...
# Function to convert character to fixed-size decimal
